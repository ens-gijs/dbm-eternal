...
| `io.github.ensgijs.dbm.repository` | `Repository`, `RepositoryApi` (annotation), `RepositoryImpl` (annotation), `AbstractRepository`, `RepositoryRegistry` (+ `RegistrationOptions`, `ConflictMode`, `RegistrationHelper`, `RegistrationBootstrappingContext`), `RepositoryComposition`, `SqlDatabaseManager` — wraps `SqlClient` and binds it to a migration set + repository scope, `AmbiguousRepositoryApiException`, `RepositoryInitializationException`, `RepositoryNotRegisteredException` |
